...
export const FIELD_HELP: Record<string, string> = {
  ...
  "tools.sessions.visibility":
    'Controls which sessions can be targeted by sessions_list/sessions_history/sessions_send. ("siblings" default enables cross-agent sessions_send only between configured agents in agents.list; "none" keeps legacy restrictive cross-agent send behavior; for sessions_list/sessions_history, "siblings"/"none" behave like legacy "tree"; "self" = only current; "agent" = any session in current agent id; "all" = any session).',
  ...
  "agents.defaults.memorySearch.extraPaths":
    "Adds extra directories or .md files to the memory index beyond default memory files (relative paths resolve from workspace; shared family memory paths are included automatically). Keep paths small and intentional to avoid noisy recall.",
